...
func Bootstrap(db *db.CustomDB, rc *redis.Client, logger logger.CustomLogger, app *gin.Engine, upg *websocket.Upgrader) {
	...
	otpGenUtil := utils.CreateOTPUtil()
	...
	studentService := services.CreateStudentService(userRepo, studentRepo, adminRepo, courseRequestRepo, chatRepo, transactionManager, bcryptUtil, gomailUtil, cloudinaryUtil, jwtUtil, otpGenUtil, googleUtil, logger)
	...
	courseRequestService := services.CreateCourseRequestService(courseRequestRepo, courseRepo, courseScheduleRepo, mentorAvailabilityRepo, userRepo, studentRepo, mentorRepo, adminRepo, paymentRepo, discountRepo, additionalCostRepo, transactionManager, gomailUtil, logger)
