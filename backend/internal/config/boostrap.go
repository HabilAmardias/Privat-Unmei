...
func Bootstrap(db *db.CustomDB, rc *redis.Client, logger logger.CustomLogger, app *gin.Engine, upg *websocket.Upgrader) {
	...
	studentService := services.CreateStudentService(userRepo, studentRepo, adminRepo, courseRequestRepo, chatRepo, transactionManager, bcryptUtil, gomailUtil, cloudinaryUtil, jwtUtil, otpGenUtil, googleUtil)
