import type { CookiesData, Fetch, SameSite, SeekPaginatedResponse, ServerResponse } from '$lib/types';
import { FetchData } from '$lib/utils';
import type { CourseList } from './model';

class CoursesController {
    async getCourses(fetch: Fetch){
        const url = 'http://localhost:8080/api/v1/courses'
        const {success, res, status, message} = await FetchData(fetch, url, 'GET')
        if (!success){
            return {success, message, status}
        }
        const resBody : ServerResponse<SeekPaginatedResponse<CourseList>> = await res?.json()
