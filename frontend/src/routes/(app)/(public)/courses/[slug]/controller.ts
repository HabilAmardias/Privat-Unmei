...
class CourseDetailController {
	...
	async getCourseTopics(fetch: Fetch, id: string) {
		const url = `http://backend:8080/api/v1/courses/${id}/topics`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async getCourseDetailCategories(fetch: Fetch, id: string) {
		const url = `http://backend:8080/api/v1/courses/${id}/categories`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async getCourseReviews(fetch: Fetch, id: string, req?: Request) {
		let url = `http://backend:8080/api/v1/courses/${id}/reviews?`;
		if (req) {
	...
	async createReview(fetch: Fetch, req: Request, id: string) {
		const url = `http://backend:8080/api/v1/courses/${id}/reviews`;
		const formData = await req.formData();
