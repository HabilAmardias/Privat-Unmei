...
class CourseDetailController {
	async getCourseDetail(fetch: Fetch, id: string) {
		const url = `http://habilog.xyz/api/v1/courses/${id}`;
	...
	async getCourseTopics(fetch: Fetch, id: string) {
		const url = `http://habilog.xyz/api/v1/courses/${id}/topics`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async getCourseDetailCategories(fetch: Fetch, id: string) {
		const url = `http://habilog.xyz/api/v1/courses/${id}/categories`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async getCourseReviews(fetch: Fetch, id: string, req?: Request) {
		let url = `http://habilog.xyz/api/v1/courses/${id}/reviews?`;
		if (req) {
	...
	async createReview(fetch: Fetch, req: Request, id: string) {
		const url = `http://habilog.xyz/api/v1/courses/${id}/reviews`;
		const formData = await req.formData();
	...
	async getProfile(fetch: Fetch) {
		const url = 'http://habilog.xyz/api/v1/me';
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async messageMentor(fetch: Fetch, req: Request) {
		...
		const url = `http://habilog.xyz/api/v1/chatrooms/users/${id}`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
