...
class CreateRequestController {
	...
	async getAvailableDayOfWeek(fetch: Fetch, id: string) {
		const url = `http://localhost/api/v1/courses/${id}/mentor-availability`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async getAdditionalCost(fetch: Fetch) {
		const url = 'http://localhost/api/v1/additional-cost/operational';
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async getDiscount(fetch: Fetch, req?: Request) {
		...
		const url = `http://localhost/api/v1/discounts/final-discount/${participant}`;
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
	...
	async createRequest(fetch: Fetch, id: string, req: Request) {
		const url = `http://localhost/api/v1/courses/${id}/course-requests`;
		const formData = await req.formData();
	...
	async getMentorPayments(fetch: Fetch, id: string) {
		const url = `http://localhost/api/v1/mentors/${id}/payment-methods`;
	...
	async getMentorSchedules(fetch: Fetch, id: string) {
		const url = `http://localhost/api/v1/mentors/${id}/availability`;
		const { success, res, status, message } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, status, message };
		}
