...
import type { ChatroomInfo, MessageInfo, MentorProfile } from './model';

class ChatroomController {
	async sendMessage(fetch: Fetch, id: string, req: Request) {
		const url = `http://habilog.xyz/api/v1/chatrooms/${id}/messages`;
	...
	async getInfo(fetch: Fetch, id: string) {
		const url = `http://habilog.xyz/api/v1/chatrooms/${id}`;
	...
	async getMessages(fetch: Fetch, id: string, req?: Request) {
		let url = `http://habilog.xyz/api/v1/chatrooms/${id}/messages?`;
		if (req) {
			const formData = await req.formData();
	...
	async getProfile(fetch: Fetch) {
		const url = 'http://habilog.xyz/api/v1/mentors/me';
		const { success, message, status, res } = await FetchData(fetch, url, 'GET');
		if (!success) {
			return { success, message, status };
		}
