...
class ChatroomController {
	async sendMessage(fetch: Fetch, id: string, req: Request) {
		const url = `http://localhost:80/api/v1/chatrooms/${id}/messages`;
	...
	async getInfo(fetch: Fetch, id: string) {
		const url = `http://localhost:80/api/v1/chatrooms/${id}`;
	...
	async getMessages(fetch: Fetch, id: string, req?: Request) {
		let url = `http://localhost:80/api/v1/chatrooms/${id}/messages?`;
		if (req) {
			const formData = await req.formData();
