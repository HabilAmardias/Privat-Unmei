...
class RequestDetailController {
	async getRequestDetail(fetch: Fetch, id: string) {
		const url = `http://160.19.167.63/api/v1/mentors/me/course-requests/${id}`;
	...
	async acceptRequest(fetch: Fetch, id: string) {
		const url = `http://160.19.167.63/api/v1/course-requests/${id}/approve`;
		const { success, message, status } = await FetchData(fetch, url, 'GET');
		return { success, message, status };
	}
	async rejectRequest(fetch: Fetch, id: string) {
		const url = `http://160.19.167.63/api/v1/course-requests/${id}/reject`;
		const { success, message, status } = await FetchData(fetch, url, 'GET');
		return { success, message, status };
	}
	async confirmPayment(fetch: Fetch, id: string) {
		const url = `http://160.19.167.63/api/v1/course-requests/${id}/confirm-payment`;
		const { success, message, status } = await FetchData(fetch, url, 'GET');
		return { success, message, status };
	}
