...
class PaymentController {
	getPayments = async (fetch: Fetch, req?: Request) => {
		let url = 'http://backend:8080/api/v1/payment-methods?';
		if (req) {
			const args: string[] = [];
	...
	updatePayment = async (fetch: Fetch, req: Request) => {
		...
		const url = `http://backend:8080/api/v1/payment-methods/${id}`;
	...
	createPayment = async (fetch: Fetch, req: Request) => {
		const formData = await req.formData();
		const url = `http://backend:8080/api/v1/payment-methods`;
		const name = formData.get('name');
		const reqBody = JSON.stringify({
			payment_method_name: name
		});
